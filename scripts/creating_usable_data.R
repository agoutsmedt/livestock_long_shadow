...
direct_citations <- merge.data.table(
  direct_citations,
  documents[, .(doi, citing_id = LLS_id)],
  by.x = "doi_citing",
  by.y = "doi",
  all.x = TRUE
)
...
direct_citations <- direct_citations[
  doi_citing %in% documents$doi & cited_id != "46698",
] # keeping only citations by level 1 articles
# Save the data
write_rds(documents, file.path(data_path, "documents.rds"), compress = "gz")
write_rds(
  unique(direct_citations[, .(citing_id, cited_id)]),
