...
create_stm <- function(data, min_word_number = 200) {
  data_set <- data %>%
    mutate(
      dfm = furrr::future_map(
        data,
        ~ tidytext::cast_dfm(data = ., document, term, count_per_doc)
      )
    ) %>%
    mutate(
      stm = furrr::future_map(dfm, ~ quanteda::convert(x = ., to = "stm")),
  ...
  data_set <- data.table::data.table(data_set, list_document, list_vocab) %>%
    rename(document = list_document, vocab = list_vocab)
  ...
  if (length(too_few_vocab) > 0) {
    data_set <- data_set[
      -which(map_dbl(data_set$vocab, length) < min_word_number)
    ]
  }
  return(data_set)
}
...
create_many_models <- function(
  ...
  for (i in 1:nrow(data_set)) {
    ...
    if (verbose == TRUE) {
      message(paste0("Topic Models for Preprocessing number", i, " completed."))
    }
  }
    ...
    tuning_results <- data %>%
      mutate(
        ...
        semantic_coherence = map(
          topic_model,
          semanticCoherence(model = ., documents = document)
        ),
  ...
  if (!exists("tuning_results")) {
    ...
    for (i in 1:nrow(many_models)) {
      coherence <- semanticCoherence(
        model = data$topic_model[[i]],
        documents = data$document[[i]]
      )
      residual <- checkResiduals(
        stmobj = data$topic_model[[i]],
        documents = data$document[[i]]
      )
    ...
    tuning_results <- tuning_results %>%
      mutate(
        residual = map_dbl(list_residual, "dispersion"),
...
harmonic_mean_exclusivity_coherence <- function(
  weight,
...
plot_topicmodels_stat <- function(
  ...
  plot_summary <- ggplot(
    results_summary,
    aes(K, Value, color = as.factor(word_threshold))
  ) +
  ...
  data[,
    frex_data_1 := map(models, average_frex, w = weight_1, nb_terms = nb_terms)
  ]
  data[,
    frex_data_2 := map(models, average_frex, w = weight_2, nb_terms = nb_terms)
  ]
  mix_measure <- data[, .(word_threshold, K, frex_data_1, frex_data_2)]
  setnames(
    mix_measure,
    c("frex_data_1", "frex_data_2"),
    c(paste0("frex_mean_", weight_1), paste0("frex_mean_", weight_2))
  )

  plot_mix_measure <- mix_measure %>%
    pivot_longer(
      cols = starts_with("frex"),
      names_to = "measure",
    ...
    ggplot(aes(
      K,
      measure_value,
      color = as.factor(word_threshold),
      group = as.factor(word_threshold)
    )) +
...
calculate_score <- function(model, nb_terms = 10) {
  ...
  ldascore <- exp(logbeta) *
    (logbeta - rep(colMeans(logbeta), each = nrow(logbeta)))
  ldascore <- t(ldascore)
  ldascore <- data.table("term" = model$vocab, as.data.table(ldascore)) %>%
    pivot_longer(
      cols = starts_with("V"),
      names_to = "topic",
      values_to = "score"
...
calculate_lift <- function(model, list_terms, nb_terms = 10) {
  ...
  lift <- t(lift)
  lift <- data.table("term" = model$vocab, as.data.table(lift)) %>%
    pivot_longer(
      cols = starts_with("V"),
      names_to = "topic",
      values_to = "lift"
...
extract_top_terms_old <- function(model, nb_words = 20, weight_frex = 0.4) {
  terms <- stm::labelTopics(model, n = nb_words, frexweight = weight_frex)
  top_terms <- data.table(
    "topic" = rep(1:model$settings$dim$K, each = nb_words),
    "rank" = 1:nb_words
  )
...
ggraph_topic_correlation <- function(
  model,
  nodes = NULL,
  id_column = "id",
  top_terms = NULL,
  ...
  edges <- as.data.frame(as.matrix(correlation$poscor)) %>%
    ...
    mutate(to = str_remove(to, "V"), from = as.character(from)) %>%
    filter(weight != 0) %>%
    unique()
  ...
  graph_corr <- community_colors(
    graph_corr,
    palette = scico(
      n = length(unique((V(graph_corr)$Com_ID))),
      palette = "roma",
      begin = 0.1
    )
  )
  ...
  graph_plot <- ggraph(graph_corr, layout = "manual", x = x, y = y) +
    geom_edge_arc0(
      aes(color = color_edges, width = weight),
      strength = 0.3,
      alpha = 0.8,
      show.legend = FALSE
    ) +
    ...
    scale_fill_identity() +
    geom_node_label(
      aes(label = topic_name, fill = color),
      size = size_label,
      alpha = 0.7
    )

  list <- list("graph" = graph_corr, "plot" = graph_plot)
  return(list)
}
...
plot_frequency <- function(topics_data, model, palette = color) {
  topics_data <- topics_data %>%
    ...
    as.data.table()
  topics_data$topic <- factor(
    topics_data$topic,
    levels = topics_data[order(frequency)]$topic
  )
  ...
  ggplot(
    ...
    geom_segment(
      aes(x = 0, xend = frequency, yend = topic),
      size = 4,
      show.legend = FALSE
    ) +
    theme(legend.position = "none") +
    geom_label(
      aes(x = frequency, color = {{ palette }}, label = term_label),
      size = 4,
      alpha = 1,
      hjust = -0.01
    ) +
    ...
    labs(title = "Top Topics", x = "Frequency", y = "") +
    theme_bw()
}
