...
nodes <- read_rds(file.path(data_path, "nodes.rds"))
...
alluvial <- read_rds(file.path(data_path, glue("alluvial_{params$method}_{params$threshold}_{params$window}_{params$resolution}.rds")))
...
where $\theta_{d,k}$ is the prevalence of topic $k$ in document $d$, and $\beta_0$ and $\beta_1$ are the coefficients to estimate. We apply a b-spline transformation to the year of publication to capture non-linear effects. We can calculate the estimated prevalence of a topic for each year of publication:
...
library(ggraph)
library(ggiraph)
graph_layout <- read_rds(here::here(data_path, "topic_modelling", "graph_layout.rds"))
...
```{r}
knitr::include_graphics(glue("pictures/alluvial_{params$method}_{params$threshold}_{params$window}_{params$resolution}.png"))
```
...
most_cited_references <- cited_articles %>% 
  # filter(! cited_id %in% nodes$citing_id) %>% 
  slice_max(order_by = nb_cit, n = 10, with_ties = FALSE, by = cluster_label) %>% 
  left_join(select(documents, cited_id = LLS_id, author_first, year, title, journal, abstract, doi)) %>%
...
central_nodes <- alluvial %>% 
  ...
  left_join(select(documents, citing_id = LLS_id, author_first, year, title, journal, abstract, doi)) %>% 
...
main_journals <- alluv_com %>% 
  select(citing_id) %>% 
  inner_join(documents, by = c("citing_id" = "LLS_id")) %>%
  count(journal) %>% 
